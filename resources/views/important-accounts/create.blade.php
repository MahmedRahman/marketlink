...
@section('title', 'إضافة حساب اشتراك')
@section('page-title', 'إضافة حساب اشتراك')

@section('content')
    <div class="form-container form-container-ia">
        <div class="form-header">
            <h2>إضافة حساب اشتراك</h2>
        ...
        <form action="{{ route('important-accounts.store') }}" method="POST" class="account-form">
            @csrf

            <div class="form-group">
                <label for="site_name">اسم الموقع / الخدمة <span class="required">*</span></label>
                <input type="text" id="site_name" name="site_name" value="{{ old('site_name') }}" required placeholder="مثال: فيسبوك، أدوبي، استضافة..."
                    class="form-input">
            </div>

            <div class="form-group">
                <label for="site_url">رابط الموقع</label>
                <input type="text" id="site_url" name="site_url" value="{{ old('site_url') }}" placeholder="https://..."
                    class="form-input">
            </div>

            <div class="form-group">
                <label for="username">اسم المستخدم / البريد <span class="required">*</span></label>
                <input type="text" id="username" name="username" value="{{ old('username') }}" required placeholder="اليوزر أو الإيميل"
                    class="form-input">
            </div>

            <div class="form-group">
                <label for="password">كلمة المرور <span class="required">*</span></label>
                <input type="text" id="password" name="password" value="{{ old('password') }}" required placeholder="كلمة المرور"
                    class="form-input">
            </div>

            <div class="form-group">
                <label for="notes">ملاحظات</label>
                <textarea id="notes" name="notes" rows="3" placeholder="أي ملاحظات إضافية..." class="form-input">{{ old('notes') }}</textarea>
            </div>
            ...
            <div class="form-group">
                ...
                <select id="month" name="month" class="form-input">
                    @php
                        ...
                        $defaultMonth = old('month', date('Y-m'));
                    @endphp
                ...
                <div class="employees-checkboxes">
                    @forelse($employees as $emp)
                        <label class="checkbox-label">
                            <input type="checkbox" name="employees[]" value="{{ $emp->id }}" {{ in_array($emp->id, old('employees', [])) ? 'checked' : '' }}>
                            <span>{{ $emp->name }}</span>
                            @if($emp->phone)
            ...
            <div class="form-actions">
                <button type="submit" class="btn-submit-ia">حفظ الحساب</button>
