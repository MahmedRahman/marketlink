...
<head>
    ...
    <title>@yield('title', 'MarketLink - التسويق الرقمي والتجارة الإلكترونية بالذكاء الاصطناعي')</title>
...
<body class="min-h-screen bg-slate-50 text-slate-900 antialiased">
    <header class="border-b border-slate-200 bg-white/90 backdrop-blur sticky top-0 z-50">
        <div class="container mx-auto px-4 py-4 flex items-center justify-between max-w-6xl">
            <a href="{{ route('home') }}" class="text-xl font-bold text-slate-800 hover:text-indigo-600 transition-colors">MarketLink</a>
        </div>
    </header>
    ...
    <footer class="border-t border-slate-200 bg-slate-800 text-slate-300 mt-16">
        <div class="container mx-auto px-4 py-12 max-w-6xl">
            <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-6">
                <div>
                    <a href="{{ route('home') }}" class="text-xl font-bold text-white hover:text-indigo-300 transition-colors">MarketLink</a>
                    <p class="text-slate-400 text-sm mt-2 max-w-md">التسويق الرقمي والتجارة الإلكترونية بالذكاء الاصطناعي — فيديوهات، تصميمات، حملات، ومواقع.</p>
