...
@section('content')
    <div class="form-container">
        ...
        <form action="{{ route('employees.store') }}" method="POST" class="employee-form">
                ...
                <div class="project-cards">
                    @if($projects->count() > 0)
                        @foreach($projects as $project)
                            <input type="checkbox" name="projects[]" id="project_{{ $project->id }}" value="{{ $project->id }}" class="project-input" data-project-id="{{ $project->id }}" {{ in_array($project->id, old('projects', [])) ? 'checked' : '' }}>
            ...
            @if($projects->count() > 0)
                @foreach($projects as $project)
                    <div class="form-group project-services-container" id="services_project_{{ $project->id }}" style="display: none;">
                        ...
                        <div class="service-cards">
                            ...
                            @foreach($serviceTypes as $key => $service)
                                <input type="checkbox" name="service_types[{{ $project->id }}][]" id="service_{{ $project->id }}_{{ $key }}" value="{{ $key }}" class="service-input" {{ in_array($key, old("service_types.{$project->id}", [])) ? 'checked' : '' }}>
